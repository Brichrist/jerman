<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('vocab.index')" :active="request()->routeIs('vocab.index')">
                        {{ __('Vocab') }}
                    </x-nav-link>
                    <x-nav-link :href="route('redemittel.index')" :active="request()->routeIs('redemittel.index')">
                        {{ __('Redemittel') }}
                    </x-nav-link>
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ms-6">
                        <x-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link href="#" class="spc-btn">
                                    {{ __('ä') }}
                                </x-dropdown-link>
                                <x-dropdown-link href="#" class="spc-btn">
                                    {{ __('ö') }}
                        ...
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <div>
                                ...
                                @if (Auth::user()->expired_at)
                                    ({{ \Carbon\Carbon::now()->diffInDays(Auth::user()->expired_at, false) }} days left)
                                @endif
                            </div>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('vocab.index')" :active="request()->routeIs('vocab.index')">
                {{ __('Vocab') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('redemittel.index')" :active="request()->routeIs('redemittel.index')">
                {{ __('Redemittel') }}
            </x-responsive-nav-link>
