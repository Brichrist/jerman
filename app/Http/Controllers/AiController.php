...
class AiController extends Controller
{
    ...
    public function askAi(Request $request)
    {
        ...
        if (auth()->user()->token !== null) {
            if (auth()->user()->token < 0) {
                return response()->json([
                    'success' => true,
                    'data' => json_encode([
                        'answer' => 'Maaf saya tidak bisa menjawab pertanyaan kamu karena tokenmu habis, silahkan hubungi developer untuk membeli token'
                    ]),
                    'remaining_dollar' => round(auth()->user()->token, 4),
                    'dollar' => 0,
                    'token' => 0,
        ...
        \Log::info('AI Answer:', ['answer' => $answer]);
        if (is_object($answer)) {
        ...
        if (auth()->user()->token !== null) {
            auth()->user()->decrement('token', $this->tokensToUSD($response->usage->promptTokens, $response->usage->completionTokens));
        }
